...
/** Create a checksum. */
// NOTE: Modified for Avalanche 
// TODO : implement Avalanche checksum
...
// NOTE: Original code adapted for Avalanche
std::string Encode(Encoding encoding, const std::string& hrp, const data& values) {
    ...
    for (const auto c : values) {        
        if (c >= 32) {
            std::cerr << "Invalid character in Bech32 encoding " << c << std::endl;
            return "";
        }
        ret << CHARSET[c];
    }
